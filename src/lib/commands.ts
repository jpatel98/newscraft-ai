...
export function parseProducerInput(
  ...
  const effectiveAgentId = mentionedAgent?.id ?? fallbackAgentId;
  const effectiveAgent = getAgent(effectiveAgentId);

  if (!effectiveAgent) {
    return {
      kind: "error",
      message: `Unknown agent "${effectiveAgentId}".`,
    };
  }
  ...
  if (!extracted.cleanedText) {
    return {
      kind: "error",
      message: "Add a brief so the agent knows where to start.",
    };
  }
  ...
  return {
    ...
    commandName: effectiveAgent.mention,
    intent: "freeform",
    cleanedPrompt: extracted.cleanedText,
    siteScope: extracted.siteScope,
  };
