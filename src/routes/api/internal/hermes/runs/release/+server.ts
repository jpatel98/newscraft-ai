...
export const POST: RequestHandler = async ({ request }) => {
	...
	if (!accountId || !runId || !tenantKey || !leaseOwner || !leaseToken) {
		return json({ detail: 'run, tenant and lease fields are required' }, { status: 400 });
	}
	...
	try {
		const run = await releaseHermesRunLease(accountId, runId, leaseOwner, leaseToken);
		return json({ state: run.state });
	} catch (cause) {
		if (cause instanceof HermesRunRepositoryError && cause.code === 'stale_lease') {
			return json({ detail: 'run lease is stale', code: cause.code }, { status: 409 });
		}
		throw cause;
	}
};
