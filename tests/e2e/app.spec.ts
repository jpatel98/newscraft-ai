...
test.describe.serial('NewsCraft app shell', () => {
	test('boots a first account, protects routes, and signs back in safely', async ({ page }) => {
		...
		await expect
			.poll(() =>
				page.getByLabel('Password').evaluate((el: HTMLInputElement) => el.validity.valueMissing)
			)
			.toBe(true);
		...
		await expect
			.poll(() =>
				page.getByLabel('Password').evaluate((el: HTMLInputElement) => el.validity.tooShort)
			)
			.toBe(true);
	...
	test('creates a thread from the composer and surfaces gateway failure without crashing', async ({
		...
		await expect(page.getByText(/Couldn't reach the agent\./)).toBeVisible();
	...
	test('keeps the primary flow usable on a mobile viewport', async ({ page }) => {
		...
		await expect(page.getByLabel('Sidebar')).toBeHidden();
		expect(problems).toEqual([]);
	});
});
